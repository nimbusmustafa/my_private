...
private:
    ...
    void laserCallback(const sensor_msgs::LaserScan::ConstPtr &msg)
    {
        ...
        right = std::min(*std::min_element(msg->ranges.begin(), msg->ranges.begin() + 144), 10.0f);
        fright = std::min(*std::min_element(msg->ranges.begin() + 145, msg->ranges.begin() + 288), 10.0f);
        front = std::min(*std::min_element(msg->ranges.begin() + 289, msg->ranges.begin() + 432), 10.0f);
        fleft = std::min(*std::min_element(msg->ranges.begin() + 433, msg->ranges.begin() + 576), 10.0f);
        left = std::min(*std::min_element(msg->ranges.begin() + 577, msg->ranges.begin() + 720), 10.0f);
    }
    ...
    void turn()
    {
        if (current_target - yaw > 0.07 || current_target - yaw <= -0.07)
        {
            ...
            {
                cmd_vel.angular.z = 0.7 * std::abs(current_target - yaw);
                cmd_vel.linear.x = 0.0;
...
public:
    void run()
    {
        ros::NodeHandle nh;
         ...
         lidar_sub = nh.subscribe("/scan", 100, &navigation::laserCallback, this);
